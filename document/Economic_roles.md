...
| **Government** | **Fiscal Authority** | $o_t^g = (\mathcal{A}_t,\mathcal{E}_{t-1},W_{t-1},P_{t-1},r_{t-1}^l,r_{t-1}^d,B_{t-1})$  Wealth distribution, education distribution, wage rate, price level, lending rate, deposit_rate, debt. | $a_t^{\text{fiscal}} = ( \boldsymbol{\tau}, G_t )$<br>Tax rates, spending | GDP growth, equality, welfare |
